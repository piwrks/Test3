...
int historyBaro[] = {950,950,950,950,950,950,950,950,950,950,950,950,950,950,950,950,950,950,950,950,950,950,950,950}; //950 bis 1060
...
void initLCD() {
 ...
 myGLCD.print(":", 520, 455, 0);
  ...
  myGLCD.setFont(BVS_9); 
  myGLCD.setColor(TUERKIS); button(x21,y21,x22,y22); 
  myGLCD.setBackColor(TUERKIS);   myGLCD.setColor(WHITE);  myGLCD.print("INNEN",x21+33.5,y21+10);  
  myGLCD.setColor(TUERKIS); button(x31,y31,x32,y32);    
  myGLCD.setBackColor(TUERKIS);   myGLCD.setColor(WHITE);  myGLCD.print("AUSSEN",x31+27,y31+10); 
  myGLCD.setColor(TUERKIS); button(x41,y41,x42,y42);    
  myGLCD.setBackColor(TUERKIS);   myGLCD.setColor(WHITE);  myGLCD.print("BAROMETER",x41+16,y41+10); 
  myGLCD.setColor(TUERKIS); button(x51,y51,x52,y52);    
  myGLCD.setBackColor(TUERKIS);   myGLCD.setColor(WHITE);  myGLCD.print("HEIZUNG",x51+26,y51+10); 
...
//Clear all
 ...
 snprintf_P(sunbuf,sizeof(sunbuf),PSTR("%02d:%02d"),sunrise/60,sunrise%60);
    ...
    //Serial.println(sunbuf);
      myGLCD.print((sunbuf), 135, 310, 0);
      myFiles.load(125, 330, 70, 40, "Aufgang40_4.raw", 1 , 0);

 snprintf_P(sunbuf,sizeof(sunbuf),PSTR("%02d:%02d"),sunset/60,sunset%60);
   
   // Serial.println(sunbuf);
    myGLCD.print((sunbuf), 135, 400, 0);
    myFiles.load(125, 420, 70, 40, "Untergang40_4.raw", 1 , 0);
...
void addHistoryTempInnen(int value){

  // shift left
  for (int i = 1; i<24; i++){
    historyTempInnen[i-1] = historyTempInnen[i];
  }
  historyTempInnen[23] = value;
 
 }

void addHistoryTempAussen(int value){

  // shift left
  for (int i = 1; i<24; i++){
    historyTempAussen[i-1] = historyTempAussen[i];
  }
  historyTempAussen[23] = value;
 
 }
 
void addHistoryHumInnen(int value){

  // shift left
  for (int i = 1; i<24; i++){
    historyHumInnen[i-1] = historyHumInnen[i];
  }
  historyHumInnen[23] = value;
 
 }
 
void addHistoryHumAussen(int value){

  // shift left
  for (int i = 1; i<24; i++){
    historyHumAussen[i-1] = historyHumAussen[i];
  }
  historyHumAussen[23] = value;
